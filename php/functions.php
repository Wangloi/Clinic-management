 ...
 */
function generateLogsTable($pdo, $sort = 'timestamp-desc', $limit = 10) {
    try {
        // Determine ORDER BY clause based on sort parameter
        $orderBy = 'sl.timestamp DESC';
        switch ($sort) {
            case 'timestamp-asc':
                $orderBy = 'sl.timestamp ASC';
                break;
            case 'timestamp-desc':
                $orderBy = 'sl.timestamp DESC';
                break;
            case 'action-asc':
                $orderBy = 'sl.action ASC, sl.timestamp DESC';
                break;
            case 'action-desc':
                $orderBy = 'sl.action DESC, sl.timestamp DESC';
                break;
            case 'user-asc':
                $orderBy = 'user_email ASC, sl.timestamp DESC';
                break;
            case 'user-desc':
                $orderBy = 'user_email DESC, sl.timestamp DESC';
                break;
            default:
                $orderBy = 'sl.timestamp DESC';
        }
            ...
            foreach ($logs as $log) {
                ...
                switch (strtolower($log['action'])) {
                    ...
                    case 'login':
                        ...
                        break;
                    case 'add student':
                        $actionClass = 'bg-green-200 text-green-900';
                        break;
                    case 'delete student':
                        $actionClass = 'bg-red-200 text-red-900';
                        break;
                    case 'edit student':
                        $actionClass = 'bg-blue-200 text-blue-900';
                        break;
                    default:
