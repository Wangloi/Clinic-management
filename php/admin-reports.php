...
<html lang="en">
        ...
        <script>
            ...
            function openViewProfileModal() {
                ...
                fetch('get_profile.php')
                    ...
                    .then(data => {
                        if (data.success) {
                            document.getElementById('viewProfileName').textContent = data.user.name || 'N/A';
                            document.getElementById('viewProfileUsername').textContent = data.user.username || 'N/A';
                            document.getElementById('viewProfileEmail').textContent = data.user.email || 'N/A';
                            document.getElementById('viewProfileRole').textContent = data.user.role || 'N/A';
                            document.getElementById('viewProfileCreated').textContent = data.user.created_at ? new Date(data.user.created_at).toLocaleDateString() : 'N/A';
                            document.getElementById('viewProfileLastLogin').textContent = data.user.last_login ? new Date(data.user.last_login).toLocaleString() : 'N/A';
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6 report-section" id="staff-activity-report">
                        ...
                        <div>
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-3 gap-4">
                                <h4 class="text-lg font-semibold text-gray-700">System Activity Logs</h4>
                                ...
                                <form method="GET" class="flex gap-3">
                                    <select name="sort" class="text-[12px] md:text-[14px] px-2 py-1 border rounded focus:outline-none focus:ring-1 focus:ring-blue-500" onchange="this.form.submit()">
                                        <option value="timestamp-desc" <?php echo ($_GET['sort'] ?? 'timestamp-desc') === 'timestamp-desc' ? 'selected' : ''; ?>>Newest First</option>
                                        <option value="timestamp-asc" <?php echo ($_GET['sort'] ?? 'timestamp-desc') === 'timestamp-asc' ? 'selected' : ''; ?>>Oldest First</option>
                                        <option value="action-asc" <?php echo ($_GET['sort'] ?? 'timestamp-desc') === 'action-asc' ? 'selected' : ''; ?>>Action A-Z</option>
                                        <option value="action-desc" <?php echo ($_GET['sort'] ?? 'timestamp-desc') === 'action-desc' ? 'selected' : ''; ?>>Action Z-A</option>
                                        <option value="user-asc" <?php echo ($_GET['sort'] ?? 'timestamp-desc') === 'user-asc' ? 'selected' : ''; ?>>User A-Z</option>
                                        <option value="user-desc" <?php echo ($_GET['sort'] ?? 'timestamp-desc') === 'user-desc' ? 'selected' : ''; ?>>User Z-A</option>
                                    </select>
                                    <select name="limit" class="text-[12px] md:text-[14px] px-2 py-1 border rounded focus:outline-none focus:ring-1 focus:ring-blue-500" onchange="this.form.submit()">
                                        <?php
                                        $limitOptions = [10, 25, 50, 100, 250];
                                        $currentLimit = $_GET['limit'] ?? 10;
                                        foreach ($limitOptions as $option) {
                                            $selected = ($option == $currentLimit) ? 'selected' : '';
                                            echo "<option value='{$option}' {$selected}>{$option} rows</option>";
                                        }
                                        ?>
                                    </select>
                                </form>
    ...
    <!-- View Profile Modal -->
    <div id="viewProfileModal" class="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm hidden z-50 flex items-center justify-center p-4">
            ...
            <div class="p-6">
                <div class="space-y-4">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h4 class="text-lg font-medium text-gray-900"><?php echo htmlspecialchars($user['fname'] . ' ' . $user['mname'] . ' ' . $user['lname']); ?></h4>
